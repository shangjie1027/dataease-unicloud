...
import io.dataease.commons.constants.SelfDsType;
...
public class DatasourceService {
    ...
    public Collection<DataSourceType> types() {
        ...
        SpringContextUtil.getApplicationContext().getBeansOfType(DataSourceType.class).values().forEach(dataSourceType -> {
            if(SelfDsType.needDsType(dataSourceType.getType()))
        ...
        SpringContextUtil.getApplicationContext().getBeansOfType(io.dataease.plugins.datasource.service.DatasourceService.class).values().forEach(datasourceService -> {
            if(SelfDsType.needDsType(datasourceService.getDataSourceType().getType()))
