...
export default {
  ...
  computed: {
    ...
    topMenuTextActiveColor() {
      if (
        ...
        return this.$store.getters.uiInfo['ui.topMenuTextActiveColor']
          .paramValue;
      }
  ...
  methods: {
    ...
    resolvePath(item) {
      ...
      // 如果是首页，就返回重定向路由
      if (item.path === '/') {
        const path =  '/#/bide/panel/index' //item.redirect;
        return path;
      }
    ...
    setSidebarHide(route) {
      ...
      if (
        route.name !== 'system' &&
        (!route.children || this.showChildLength(route) === 1)
      ) {
    ...
    async logout() {
      ...
      // this.$router.push(`/bide/login?redirect=${this.$route.fullPath}`)
      window.location.href = '/bide/login';
    ...
    loadUiInfo() {
      this.$store.dispatch('user/getUI').then(() => {
        ...
        if (this.uiInfo['ui.logo'] && this.uiInfo['ui.logo'].paramValue) {
          this.logoUrl =
            '/system/ui/image/' + this.uiInfo['ui.logo'].paramValue;
        }
