            ...
            <div class="login-form">
              <el-form-item v-if="loginTypes.length > 1">
                ...
                >
                  ...
                  <el-radio v-if="loginTypes.includes(1)" :label="1" size="mini"
                    >LDAP</el-radio
                  >
                  <el-radio v-if="loginTypes.includes(2)" :label="2" size="mini"
                    >OIDC</el-radio
                  >
                </el-radio-group>
              </el-form-item>
...
export default {
        ...
        username: [
          {
            required: true,
            trigger: 'blur',
            message: this.$t('commons.input_id'),
          },
        ],
        password: [
          {
            required: true,
            trigger: 'blur',
            message: this.$t('commons.input_pwd'),
          },
        ],
      },
      loading: false,
  ...
  methods: {
    ...
    showLoginImage() {
      ...
      if (
        this.uiInfo['ui.loginImage'] &&
        this.uiInfo['ui.loginImage'].paramValue
      ) {
        this.loginImageUrl =
          '/system/ui/image/' + this.uiInfo['ui.loginImage'].paramValue;
      }
      if (
        this.uiInfo['ui.loginLogo'] &&
        this.uiInfo['ui.loginLogo'].paramValue
      ) {
        this.loginLogoUrl =
          '/system/ui/image/' + this.uiInfo['ui.loginLogo'].paramValue;
      }
    ...
    handleLogin() {
      ...
      console.log('我已进入登录页面-handlelogin');
      this.initCache();
      this.clearOidcMsg();
      ...
      this.$store
        ...
        .then(() => {
          console.log('登录成功-success');        
            window.location.href = '/#/bide/panel/index';
            console.log('已跳转 panel');
            this.loading = false;
        })
        .catch(() => {
          console.log('异常跳转 panel');
          window.location.href = '/#/bide/panel/index';
          this.loading = false;
        });
      //   } else {
